...
function deduplicar(contatos) {
    return newContatos = contatos.reduce((acc, contato) => {
        let contatoNome = contato.nome ? contato.nome.toLowerCase() : null
        let contatoEmail = contato.email ? contato.email.toLowerCase() : null
        for(newContato of acc) {
            if(newContato.nome == contatoNome || newContato.email == contatoEmail) {
                newContato.nome = newContato.nome ?? contatoNome
                newContato.email = newContato.email ?? contatoEmail
                newContato.telefone = newContato.telefone ?? contato.telefone
                return acc
            }
        }
        //Certifica que está tudo inserido em lowerCase
        acc.push({nome: contatoNome, email: contatoEmail, telefone: contato.telefone})
        return acc
    }, acc = [])
